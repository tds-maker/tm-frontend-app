...
import { editTemplateOperations, editTemplateTypes  } from "../../modules/templates/routes/EditTemplate/ducks";
...
const undoRedoMiddleware = ({ getState, dispatch }:any) => {
	return (next:any) => (action: IAction) => {
		...
		if(action.hasAudit){
			...
			dispatch(editTemplateOperations.addToHistoryNext(prevState.template.editTemplate.design))
		}else if(action.type === editTemplateTypes.REDO){
			dispatch(editTemplateOperations.addToHistoryPrev(prevState.template.editTemplate.design));
		}
