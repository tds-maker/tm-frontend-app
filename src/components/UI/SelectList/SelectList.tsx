...
class SelectList extends React.PureComponent<IProps, IState> {
	...
	public componentDidUpdate() {
		if (this.state.isOpen && this.props.items && this.props.items.length > 5 && this.wrapperRef) {
		...
		return (
			<div className={parentClassnames} style={{ width }} ref={this.setWrapperRef}>
				<div className="current" onClick={this.toggleOpenStatus}>
					{!selectedItem ? <span className="placeholder">{placeholder}</span> : <span className="text">{selectedItem.text}</span>}
				</div>
	...
	private renderItem(item: ISelectItem, index: number) {
		...
		return (
			<li key={index} className={itemClassnames} onClick={this.onItemSelected.bind(this, item)}>
				{item.text}
			</li>
		);
	}
