...
class TemplateReducer extends BaseReducer<Template> {
	...
	public setLanguages(
		state: Template,
		payload: { isSelected: boolean; language: string }
	): Template {
		...
		if (payload.isSelected && primaryLanguage) {
			newState = state.updateIn(['languages'], languages =>
				languages.push(payload.language)
			) as Template
		} else if (payload.isSelected && !primaryLanguage) {
			newState = state
				.updateIn(['languages'], languages => languages.push(payload.language))
				.set('primaryLanguage', payload.language) as Template
		} else if (!payload.isSelected && payload.language !== primaryLanguage) {
			newState = state.updateIn(['languages'], languages =>
				languages.splice(languages.indexOf(payload.language), 1)
			) as Template
