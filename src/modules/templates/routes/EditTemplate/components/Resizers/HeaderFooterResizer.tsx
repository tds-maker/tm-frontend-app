...
class HeaderFooterResizer extends React.PureComponent<IProps, IState> {
    ...
    public render(){
        const styleHeight = `${this.state.isResizing ? this.state.height: this.props.size.height}px`;
        return (<div className="header-footer-resizer" style={{height: styleHeight}}>
            {this.props.children}

            {this.props.directions.includes('s') ? 
                <div id="bottom" className="header-footer-resizer__bottom-line" onMouseDown={this.onMouseDown} />:null
            }
            {this.props.directions.includes('n') ? 
                <div id="top" className="header-footer-resizer__top-line" onMouseDown={this.onMouseDown} />:null
            }
        </div>)
    }
    ...
    private onMouseDown(e: any){
        if(!this.state.isResizing){
            ...
            this.setState({ isResizing : true, height: this.props.size.height, handler: e.target.id });
        }
    }
