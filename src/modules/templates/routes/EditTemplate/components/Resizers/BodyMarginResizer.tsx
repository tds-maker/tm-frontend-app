...
const BodyMarginResizerHOC = (WrappedComponent:any, options: IBodyResizerOptions) => {
    return class Resizer extends React.Component<any, IState>{
        ...
        private onMouseUp(e: MouseEvent){
            if(this.state.isResizing){
                if(this.props[options.onResize] && typeof this.props[options.onResize] === 'function'){
        ...
        private styleMapper(onlyParent = false){
            ...
            if(options.direction){
                Object.keys(options.direction).forEach((key:string) => {
                    ...
                    style[value] = (onlyParent || !this.state.isResizing) ? parentStyle[value] : this.getChanges(value, parentStyle, this.state.changes) ;
                })
            }
            return style;
        }
...
export const BodyMarginResizerHandlers  = ({onMouseDown, hasHeader, hasFooter}: IBodyMarginResizerHandlersProps ) => (
    <React.Fragment>
            {!hasHeader ?
                <div id="top" onMouseDown={onMouseDown} style={{position: 'absolute', cursor:'row-resize', top: "-1px",  right:"0", left:"0", height: "10px", borderTop: "1px dashed rgba(0, 0, 0, 0.3)"}} /> : null
            }
            {!hasFooter ? 
                <div id="bottom" onMouseDown={onMouseDown} style={{position: 'absolute', cursor:'row-resize', bottom: "-1px",  right:"0", left:"0", height: "10px", borderBottom: "1px dashed rgba(0, 0, 0, 0.3)"}} />:null
            }
            
            <div id="left" onMouseDown={onMouseDown} style={{position: 'absolute', cursor:'col-resize', top: "0",  bottom:"0", left:"-1px", width: "10px", borderLeft: "1px dashed rgba(0, 0, 0, 0.3)"}} />
            <div id="right" onMouseDown={onMouseDown} style={{position: 'absolute', cursor:'col-resize', top: "0",  bottom:"0", right:"-1px", width: "10px", borderRight: "1px dashed rgba(0, 0, 0, 0.3)"}} />
    </React.Fragment>
)
