...
class FoldersTree extends React.Component<IProps> {
	...
	public render() {
		...
		return <ul className="tm-folders">{[rootFolder].map(this.renderFolderItem.bind(this, selectedFolder))}</ul>;
	}
	...
	private renderFolderItem(selectedFolder: Folder, folder: Folder): JSX.Element | null {
		...
		return (
			<li key={folder._id} className={folderClassNames} onClick={this.onFolderClick.bind(this, folder._id)}>
				<span className="noselect">{folder.name}</span>
				{folder.isOpen && childNodes && childNodes.size > 0 ? <ul style={{ display: true ? "block" : "none" }}>{childNodes.map(this.renderFolderItem.bind(this, selectedFolder))}</ul> : null}
			</li>
		);
	}
